...
export default function ShoppingMoneyVocabulary() {
  ...
  const fetchVocabularyData = async (lang: string) => {
    setLoading(true);
    try {
      const response = await fetch(`/json_files/vocabulary_shopping_money_${lang}.json`);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-300/40 via-white to-emerald-300/40">
        <Navbar />
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-8">
              ...
              <div className="absolute right-0 mt-2 w-48 bg-white/90 backdrop-blur-xl rounded-xl shadow-xl border border-white/30 z-50">
                {languages.map((lang) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-green-50/80 transition first:rounded-t-xl last:rounded-b-xl ${
                      selectedLang === lang.code ? 'bg-green-50/80' : ''
                    }`}
                  >
                    <span>{lang.flag}</span>
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-100 rounded-full mb-4">
            ...
            <span className="text-sm font-medium text-green-700">Shopping & Money</span>
          </div>
          <h1 className="text-4xl font-black text-slate-900 mb-3">
            Shopping and Money
          </h1>
          <p className="text-slate-600 max-w-2xl mx-auto">
            Learn essential German vocabulary for shopping, stores, transactions, and money management.
          </p>
        </div>
          ...
          {vocabularyData?.sections.map((section, sectionIndex) => (
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-slate-900">Shopping & Money Vocabulary</h2>
              </div>
                    ...
                    <div className="space-y-4">
                      ...
                      {item.article && (
                        <div className="inline-flex items-center gap-2 px-3 py-1 bg-green-100 rounded-full">
                          <span className="text-sm font-bold text-green-700">{item.article}</span>
        ...
        <div className="mt-12 text-center">
          <div className="backdrop-blur-xl bg-white/40 border border-white/30 rounded-3xl p-8 shadow-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
              <div className="space-y-2">
                <h3 className="font-bold text-green-700">🛍️ Shopping Phrases</h3>
                <p className="text-sm text-slate-600">
                  Practice shopping conversations: "Ich möchte kaufen" (I would like to buy).
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-green-700">💰 Money Talk</h3>
                <p className="text-sm text-slate-600">
                  Learn price discussions: "Wie viel kostet das?" (How much does it cost?).
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-green-700">🏪 Store Types</h3>
                <p className="text-sm text-slate-600">
                  Identify different stores: "der Supermarkt" (supermarket), "die Bäckerei" (bakery).
                </p>
              </div>
            </div>
          </div>
