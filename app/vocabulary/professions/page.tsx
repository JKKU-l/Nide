...
export default function ProfessionsVocabulary() {
  ...
  const fetchVocabularyData = async (lang: string) => {
    setLoading(true);
    try {
      const response = await fetch(`/json_files/vocabulary_professions_${lang}.json`);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-teal-300/40 via-white to-cyan-300/40">
        <Navbar />
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-8">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-teal-100 rounded-full mb-4">
            ...
            <span className="text-sm font-medium text-teal-700">Professions</span>
          </div>
          <h1 className="text-4xl font-black text-slate-900 mb-3">
            Professions
          </h1>
          <p className="text-slate-600 max-w-2xl mx-auto">
            Learn essential German vocabulary for various professions and occupations.
          </p>
        </div>
          ...
          {vocabularyData?.sections.map((section, sectionIndex) => (
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-slate-900">Professions Vocabulary</h2>
              </div>
                    ...
                    <div className="space-y-4">
                      ...
                      {item.article && (
                        <div className="inline-flex items-center gap-2 px-3 py-1 bg-teal-100 rounded-full">
                          <span className="text-sm font-bold text-teal-700">{item.article}</span>
        ...
        <div className="mt-12 text-center">
          <div className="backdrop-blur-xl bg-white/40 border border-white/30 rounded-3xl p-8 shadow-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
              <div className="space-y-2">
                <h3 className="font-bold text-teal-700">💼 Career Talk</h3>
                <p className="text-sm text-slate-600">
                  Practice discussing professions: "Ich bin Bäcker" (I am a baker).
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-teal-700">🔧 Technical Jobs</h3>
                <p className="text-sm text-slate-600">
                  Learn technical professions: "Er ist Elektriker" (He is an electrician).
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-teal-700">🏠 Home Roles</h3>
                <p className="text-sm text-slate-600">
                  Understand domestic roles: "Sie ist Hausfrau" (She is a homemaker).
                </p>
              </div>
            </div>
          </div>
