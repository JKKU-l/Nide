...
export default function KitchenTablewareVocabulary() {
  ...
  const fetchVocabularyData = async (lang: string) => {
    setLoading(true);
    try {
      const response = await fetch(`/json_files/vocabulary_kitchen_tableware_${lang}.json`);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-cyan-300/40 via-white to-teal-300/40">
        <Navbar />
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-8">
              ...
              <div className="absolute right-0 mt-2 w-48 bg-white/90 backdrop-blur-xl rounded-xl shadow-xl border border-white/30 z-50">
                {languages.map((lang) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-cyan-50/80 transition first:rounded-t-xl last:rounded-b-xl ${
                      selectedLang === lang.code ? 'bg-cyan-50/80' : ''
                    }`}
                  >
                    <span>{lang.flag}</span>
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-cyan-100 rounded-full mb-4">
            ...
            <span className="text-sm font-medium text-cyan-700">Kitchen & Tableware</span>
          </div>
          <h1 className="text-4xl font-black text-slate-900 mb-3">
            Kitchen and Tableware
          </h1>
          <p className="text-slate-600 max-w-2xl mx-auto">
            Learn essential German vocabulary for kitchen items, tableware, cooking, and dining experiences.
          </p>
        </div>
          ...
          {vocabularyData?.sections.map((section, sectionIndex) => (
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-slate-900">Kitchen & Tableware Vocabulary</h2>
              </div>
                    ...
                    <div className="space-y-4">
                      ...
                      {item.article && (
                        <div className="inline-flex items-center gap-2 px-3 py-1 bg-cyan-100 rounded-full">
                          <span className="text-sm font-bold text-cyan-700">{item.article}</span>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="w-8 h-8 rounded-full bg-cyan-100 flex items-center justify-center hover:bg-cyan-200 transition"
                          >
        ...
        <div className="mt-12 text-center">
          <div className="backdrop-blur-xl bg-white/40 border border-white/30 rounded-3xl p-8 shadow-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
              <div className="space-y-2">
                <h3 className="font-bold text-cyan-700">🍳 Kitchen Actions</h3>
                <p className="text-sm text-slate-600">
                  Practice cooking verbs like "kochen" (to cook) and describe what you're making in the kitchen.
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-cyan-700">🍽️ Table Settings</h3>
                <p className="text-sm text-slate-600">
                  Learn tableware vocabulary by setting a table and naming each item: "der Teller", "die Gabel", "das Messer".
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-cyan-700">👅 Taste Descriptions</h3>
                <p className="text-sm text-slate-600">
                  Master taste adjectives: "süß" (sweet), "sauer" (sour), "scharf" (spicy), "salzig" (salty).
                </p>
              </div>
            </div>
          </div>
