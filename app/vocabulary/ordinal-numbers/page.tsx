...
export default function OrdinalNumbersVocabulary() {
  ...
  const fetchVocabularyData = async (lang: string) => {
    setLoading(true);
    try {
      const response = await fetch(`/json_files/vocabulary_ordinal_numbers_${lang}.json`);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-300/40 via-white to-amber-300/40">
        <Navbar />
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-8">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-orange-100 rounded-full mb-4">
            ...
            <span className="text-sm font-medium text-orange-700">Ordinal Numbers</span>
          </div>
          <h1 className="text-4xl font-black text-slate-900 mb-3">
            Ordinal Numbers
          </h1>
          <p className="text-slate-600 max-w-2xl mx-auto">
            Learn essential German vocabulary for ordinal numbers to describe positions, rankings, and sequences.
          </p>
        </div>
          ...
          {vocabularyData?.sections.map((section, sectionIndex) => (
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-slate-900">Ordinal Numbers Vocabulary</h2>
              </div>
        ...
        <div className="mt-12 text-center">
          <div className="backdrop-blur-xl bg-white/40 border border-white/30 rounded-3xl p-8 shadow-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
              <div className="space-y-2">
                <h3 className="font-bold text-orange-700">🏆 Rankings</h3>
                <p className="text-sm text-slate-600">
                  Practice ranking things: "Das ist das erste Auto" (This is the first car).
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-orange-700">📅 Dates</h3>
                <p className="text-sm text-slate-600">
                  Use ordinals for dates: "der erste Mai" (the first of May).
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-orange-700">📍 Positions</h3>
                <p className="text-sm text-slate-600">
                  Describe positions: "Ich bin der zweite in der Reihe" (I'm second in line).
                </p>
              </div>
            </div>
          </div>
