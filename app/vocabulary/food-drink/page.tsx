...
export default function FoodDrinkVocabulary() {
  ...
  const fetchVocabularyData = async (lang: string) => {
    setLoading(true);
    try {
      const response = await fetch(`/json_files/vocabulary_food_drink_${lang}.json`);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-300/40 via-white to-red-300/40">
        <Navbar />
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-8">
              ...
              <div className="absolute right-0 mt-2 w-48 bg-white/90 backdrop-blur-xl rounded-xl shadow-xl border border-white/30 z-50">
                {languages.map((lang) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-orange-50/80 transition first:rounded-t-xl last:rounded-b-xl ${
                      selectedLang === lang.code ? 'bg-orange-50/80' : ''
                    }`}
                  >
                    <span>{lang.flag}</span>
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-orange-100 rounded-full mb-4">
            ...
            <span className="text-sm font-medium text-orange-700">Food & Drink</span>
          </div>
          <h1 className="text-4xl font-black text-slate-900 mb-3">
            Food and Drink
          </h1>
          <p className="text-slate-600 max-w-2xl mx-auto">
            Learn essential German vocabulary for food, drinks, meals, and everything related to dining and nutrition.
          </p>
        </div>
          ...
          {vocabularyData?.sections.map((section, sectionIndex) => (
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-slate-900">Food & Drink Vocabulary</h2>
              </div>
                    ...
                    <div className="space-y-4">
                      ...
                      {item.article && (
                        <div className="inline-flex items-center gap-2 px-3 py-1 bg-orange-100 rounded-full">
                          <span className="text-sm font-bold text-orange-700">{item.article}</span>
        ...
        <div className="mt-12 text-center">
          <div className="backdrop-blur-xl bg-white/40 border border-white/30 rounded-3xl p-8 shadow-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
              <div className="space-y-2">
                <h3 className="font-bold text-orange-700">🍽️ Daily Meals</h3>
                <p className="text-sm text-slate-600">
                  Practice describing your daily meals using food vocabulary. Start with "Ich esse..." (I eat...).
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-orange-700">🛒 Grocery Shopping</h3>
                <p className="text-sm text-slate-600">
                  Learn shopping phrases like "Ich möchte..." (I would like...) when ordering food or buying groceries.
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-orange-700">🍳 Cooking Verbs</h3>
                <p className="text-sm text-slate-600">
                  Focus on cooking-related verbs like "essen" (to eat), "trinken" (to drink), and "backen" (to bake).
                </p>
              </div>
            </div>
          </div>
