...
export default function ClothingAppearance() {
  ...
  const fetchVocabularyData = async (lang: string) => {
    setLoading(true);
    try {
      const response = await fetch(`/json_files/clothing_appearance_${lang}.json`);
  ...
  const separableVerbs: Record<string, { prefix: string; stem: string }> = {
    ...
    'umziehen': { prefix: 'um', stem: 'ziehen' },
    'anprobieren': { prefix: 'an', stem: 'probieren' },
    'aufhängen': { prefix: 'auf', stem: 'hängen' },
          ...
          {vocabularyData?.sections.map((section, sectionIndex) => (
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-slate-900">Clothing & Appearance Vocabulary</h2>
              </div>
          ...
          <div className="backdrop-blur-xl bg-white/40 border border-white/30 rounded-3xl p-8 shadow-xl">
            ...
            <div className="mb-6 p-4 bg-blue-50 rounded-xl">
              ...
              <div className="flex flex-wrap gap-4 justify-center">
                <div className="flex items-center gap-2">
                  <span className="text-red-600 font-bold">an</span>
                  <span className="text-sm text-slate-600">= Separable prefix</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-blue-600 font-bold">ziehen</span>
                  <span className="text-sm text-slate-600">= Verb stem</span>
                </div>
              </div>
              <p className="text-sm text-slate-500 mt-2">
                In German, separable verbs split when used in sentences: "Ich <span className="text-blue-600 font-bold">ziehe</span> mir eine Jacke <span className="text-red-600 font-bold">an</span>"
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
              <div className="space-y-2">
                <h3 className="font-bold text-blue-700">👔 Clothing Actions</h3>
                <p className="text-sm text-slate-600">
                  Learn clothing verbs: "anziehen" (to put on), "ausziehen" (to take off), "tragen" (to wear).
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-blue-700">🧺 Laundry Care</h3>
                <p className="text-sm text-slate-600">
                  Practice laundry vocabulary: "waschen" (to wash), "bügeln" (to iron), "trocknen" (to dry).
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-blue-700">🛍️ Shopping</h3>
                <p className="text-sm text-slate-600">
                  Use shopping verbs: "kaufen" (to buy), "anprobieren" (to try on), "passen" (to fit).
                </p>
              </div>
            </div>
          </div>
