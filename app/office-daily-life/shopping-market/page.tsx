...
export default function ShoppingMarket() {
  ...
  const fetchVocabularyData = async (lang: string) => {
    setLoading(true);
    try {
      const response = await fetch(`/json_files/shopping_market_${lang}.json`);
  ...
  const separableVerbs: Record<string, { prefix: string; stem: string }> = {
    'einkaufen': { prefix: 'ein', stem: 'kaufen' },
    'anprobieren': { prefix: 'an', stem: 'probieren' },
    'zurückgeben': { prefix: 'zurück', stem: 'geben' },
    'umtauschen': { prefix: 'um', stem: 'tauschen' },
    'abholen': { prefix: 'ab', stem: 'holen' },
    'einpacken': { prefix: 'ein', stem: 'packen' },
    'anschauen': { prefix: 'an', stem: 'schauen' },
    'mitnehmen': { prefix: 'mit', stem: 'nehmen' },
    'ausgeben': { prefix: 'aus', stem: 'geben' },
  };
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-8">
              ...
              <div className="absolute right-0 mt-2 w-48 bg-white/90 backdrop-blur-xl rounded-xl shadow-xl border border-white/30 z-50">
                {languages.map((lang) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-teal-50/80 transition first:rounded-t-xl last:rounded-b-xl ${
                      selectedLang === lang.code ? 'bg-teal-50/80' : ''
                    }`}
                  >
                    <span>{lang.flag}</span>
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-teal-100 rounded-full mb-4">
            ...
            <span className="text-sm font-medium text-teal-700">{vocabularyData?.title}</span>
          </div>
          ...
          {vocabularyData?.sections.map((section, sectionIndex) => (
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-slate-900">Shopping & Market Vocabulary</h2>
              </div>
                        ...
                        <div className="border-t border-teal-200 pt-4 space-y-2">
                          ...
                          <div className="space-y-2">
                            <div className="flex items-start gap-2">
                              <span className="text-xs font-bold text-teal-600 mt-1">DE:</span>
                              <div className="flex-1 flex items-start gap-2">
                                ...
                                <button
                                  ...
                                  className="w-6 h-6 rounded-full bg-teal-100 flex items-center justify-center hover:bg-teal-200 transition flex-shrink-0 mt-0.5"
                                >
          ...
          <div className="backdrop-blur-xl bg-white/40 border border-white/30 rounded-3xl p-8 shadow-xl">
            ...
            <div className="mb-6 p-4 bg-teal-50 rounded-xl">
              ...
              <div className="flex flex-wrap gap-4 justify-center">
                <div className="flex items-center gap-2">
                  <span className="text-red-600 font-bold">ein</span>
                  <span className="text-sm text-slate-600">= Separable prefix</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-blue-600 font-bold">kaufen</span>
                  <span className="text-sm text-slate-600">= Verb stem</span>
                </div>
              </div>
              <p className="text-sm text-slate-500 mt-2">
                In German, separable verbs split when used in sentences: &quot;Ich <span className="text-blue-600 font-bold">kaufe</span> für das Abendessen <span className="text-red-600 font-bold">ein</span>&quot;
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
              <div className="space-y-2">
                <h3 className="font-bold text-teal-700">🛒 Shopping Basics</h3>
                <p className="text-sm text-slate-600">
                  Master essential verbs: &quot;einkaufen&quot; (to shop), &quot;kaufen&quot; (to buy), &quot;bezahlen&quot; (to pay).
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-teal-700">💰 Money & Payments</h3>
                <p className="text-sm text-slate-600">
                  Learn financial verbs: &quot;sparen&quot; (to save), &quot;ausgeben&quot; (to spend), &quot;wechseln&quot; (to exchange).
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-teal-700">🏪 At the Store</h3>
                <p className="text-sm text-slate-600">
                  Practice store interactions: &quot;fragen&quot; (to ask), &quot;empfehlen&quot; (to recommend), &quot;helfen&quot; (to help).
                </p>
              </div>
            </div>
          </div>
