...
export default function FamilyRelationships() {
  ...
  const fetchVocabularyData = async (lang: string) => {
    setLoading(true);
    try {
      const response = await fetch(`/json_files/family_relationships_${lang}.json`);
          ...
          {vocabularyData?.sections.map((section, sectionIndex) => (
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-slate-900">Family & Relationships Vocabulary</h2>
              </div>
        ...
        <div className="mt-12 text-center">
          <div className="backdrop-blur-xl bg-white/40 border border-white/30 rounded-3xl p-8 shadow-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
              <div className="space-y-2">
                <h3 className="font-bold text-blue-700">👨‍👩‍👧‍👦 Family Members</h3>
                <p className="text-sm text-slate-600">
                  Learn family vocabulary: "Vater" (father), "Mutter" (mother), "Bruder" (brother), "Schwester" (sister).
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-blue-700">💑 Relationships</h3>
                <p className="text-sm text-slate-600">
                  Practice relationship terms: "Ehepartner" (spouse), "Freund" (friend), "Kollege" (colleague).
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-blue-700">🗣️ Family Conversations</h3>
                <p className="text-sm text-slate-600">
                  Use family vocabulary in conversations: "Wie geht es deiner Familie?" (How is your family?).
                </p>
              </div>
            </div>
          </div>
