...
export default function SocialLifeCommunity() {
  ...
  const fetchVocabularyData = async (lang: string) => {
    setLoading(true);
    try {
      const response = await fetch(`/json_files/social_life_community_${lang}.json`);
  ...
  const separableVerbs: Record<string, { prefix: string; stem: string }> = {
    'mitbringen': { prefix: 'mit', stem: 'bringen' },
    'vorstellen': { prefix: 'vor', stem: 'stellen' },
  };
          ...
          {vocabularyData?.sections.map((section, sectionIndex) => (
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-slate-900">Social Life & Community Vocabulary</h2>
              </div>
          ...
          <div className="backdrop-blur-xl bg-white/40 border border-white/30 rounded-3xl p-8 shadow-xl">
            ...
            <div className="mb-6 p-4 bg-blue-50 rounded-xl">
              ...
              <div className="flex flex-wrap gap-4 justify-center">
                <div className="flex items-center gap-2">
                  <span className="text-red-600 font-bold">mit</span>
                  <span className="text-sm text-slate-600">= Separable prefix</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-blue-600 font-bold">bringen</span>
                  <span className="text-sm text-slate-600">= Verb stem</span>
                </div>
              </div>
              <p className="text-sm text-slate-500 mt-2">
                In German, separable verbs split when used in sentences: "Ich <span className="text-blue-600 font-bold">bringe</span> etwas <span className="text-red-600 font-bold">mit</span>"
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
              <div className="space-y-2">
                <h3 className="font-bold text-blue-700">👋 Social Interactions</h3>
                <p className="text-sm text-slate-600">
                  Practice social phrases: "Hallo", "Wie geht's?", "Freut mich".
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-blue-700">🎉 Celebrations</h3>
                <p className="text-sm text-slate-600">
                  Learn celebration vocabulary: "feiern" (to celebrate), "tanzen" (to dance).
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-blue-700">🗣️ Communication</h3>
                <p className="text-sm text-slate-600">
                  Practice communication: "reden" (to talk), "vorstellen" (to introduce).
                </p>
              </div>
            </div>
          </div>
