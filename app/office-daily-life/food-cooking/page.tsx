...
export default function FoodCooking() {
  ...
  const fetchVocabularyData = async (lang: string) => {
    setLoading(true);
    try {
      const response = await fetch(`/json_files/food_cooking_${lang}.json`);
  ...
  const separableVerbs: Record<string, { prefix: string; stem: string }> = {
    'anbraten': { prefix: 'an', stem: 'braten' },
  };
          ...
          {vocabularyData?.sections.map((section, sectionIndex) => (
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-slate-900">Food & Cooking Vocabulary</h2>
              </div>
          ...
          <div className="backdrop-blur-xl bg-white/40 border border-white/30 rounded-3xl p-8 shadow-xl">
            ...
            <div className="mb-6 p-4 bg-blue-50 rounded-xl">
              ...
              <div className="flex flex-wrap gap-4 justify-center">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-blue-600 font-bold">braten</span>
                  <span className="text-sm text-slate-600">= Verb stem</span>
                </div>
              </div>
              <p className="text-sm text-slate-500 mt-2">
                In German, separable verbs split when used in sentences: "Ich <span className="text-blue-600 font-bold">brate</span> das Fleisch <span className="text-red-600 font-bold">an</span>"
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
              <div className="space-y-2">
                <h3 className="font-bold text-blue-700">🔪 Preparation</h3>
                <p className="text-sm text-slate-600">
                  Learn preparation verbs: "schneiden" (to cut), "schälen" (to peel), "mischen" (to mix).
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-blue-700">🍳 Cooking Methods</h3>
                <p className="text-sm text-slate-600">
                  Practice cooking techniques: "kochen" (to boil), "braten" (to fry), "backen" (to bake).
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-blue-700">🍽️ Kitchen Verbs</h3>
                <p className="text-sm text-slate-600">
                  Use kitchen vocabulary: "schmecken" (to taste), "anrichten" (to serve), "auftischen" (to dish up).
                </p>
              </div>
            </div>
          </div>
