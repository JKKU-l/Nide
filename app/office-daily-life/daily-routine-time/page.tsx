...
export default function DailyRoutineTime() {
  ...
  const fetchVocabularyData = async (lang: string) => {
    setLoading(true);
    try {
      const response = await fetch(`/json_files/daily_routine_time_${lang}.json`);
  ...
  const separableVerbs: Record<string, { prefix: string; stem: string }> = {
    ...
    'einschlafen': { prefix: 'ein', stem: 'schlafen' },
    'anfangen': { prefix: 'an', stem: 'fangen' },
    'aufhören': { prefix: 'auf', stem: 'hören' },
    'zurückkommen': { prefix: 'zurück', stem: 'kommen' },
    'eilen': { prefix: 'be', stem: 'eilen' },
    'vorbereiten': { prefix: 'vor', stem: 'bereiten' },
    'anziehen': { prefix: 'an', stem: 'ziehen' },
    'ausziehen': { prefix: 'aus', stem: 'ziehen' },
    'aufräumen': { prefix: 'auf', stem: 'räumen' },
  };
          ...
          {vocabularyData?.sections.map((section, sectionIndex) => (
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-slate-900">Daily Routine & Time Vocabulary</h2>
              </div>
          ...
          <div className="backdrop-blur-xl bg-white/40 border border-white/30 rounded-3xl p-8 shadow-xl">
            ...
            <div className="mb-6 p-4 bg-blue-50 rounded-xl">
              ...
              <p className="text-sm text-slate-500 mt-2">
                In German, separable verbs split when used in sentences: "Ich <span className="text-red-600 font-bold">stehe</span> um 7 Uhr <span className="text-red-600 font-bold">auf</span>"
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
              <div className="space-y-2">
                <h3 className="font-bold text-blue-700">⏰ Daily Routine</h3>
                <p className="text-sm text-slate-600">
                  Practice describing your daily routine: "Ich stehe um 7 Uhr auf" (I get up at 7 o'clock).
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-blue-700">📅 Time Expressions</h3>
                <p className="text-sm text-slate-600">
                  Learn common time phrases: "um 8 Uhr" (at 8 o'clock), "vor 10 Minuten" (10 minutes ago).
                </p>
              </div>
              <div className="space-y-2">
                <h3 className="font-bold text-blue-700">💬 Daily Activities</h3>
                <p className="text-sm text-slate-600">
                  Use these verbs to talk about daily activities: "Ich arbeite, ich koche, ich dusche".
                </p>
              </div>
            </div>
          </div>
