...
export default function MyMother() {
  ...
  const fetchTopicData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/json_files/my_mother_${selectedLanguage}.json`);
  ...
  const getTranslation = () => {
    ...
    switch (selectedLanguage) {
      case 'de': return content.german_translation || '';
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-8">
        ...
        <div className="flex justify-center mb-8">
          <div className="backdrop-blur-xl bg-white/40 border border-white/30 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-xl">
            <Image
              src="/cartoons/mother_and_daughter.svg"
